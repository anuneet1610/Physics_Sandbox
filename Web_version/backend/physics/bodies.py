...
class Ball(RigidBody):
    ...
    def to_dict(self):
        return {
            "id": self.id, "kind": "ball", "x": self.x, "y": self.y,
            "vx": self.vx, "vy": self.vy, "mass": self.mass,
            "radius": self.radius, "colour": self.colour,
            "fx": self.fx, "fy": self.fy,
        }

    def check_collision(self, other, e):
        dx = other.x - self.x
        dy = other.y - self.y
        dist_sq = dx * dx + dy * dy
        ...
        restitution = 0.0 if vel_along_normal < REST_THRESHOLD else e
...
class Rectangle(RigidBody):
    ...
    def to_dict(self):
        return {
            "id": self.id, "kind": "rect", "x": self.x, "y": self.y,
            "vx": self.vx, "vy": self.vy, "mass": self.mass,
            "length": self.length, "width": self.width, "colour": self.colour,
            "fx": self.fx, "fy": self.fy,
        }
    ...
    def check_collision_ball(self, ball, e):
